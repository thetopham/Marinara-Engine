...
- Added a constrained window UI to Browser Personal Extensions: `marinara.ui.showWindow(...)` lets an extension present a closable window built from a fixed set of elements (headings, text, preformatted blocks, buttons, and inputs), with button clicks reporting current input values back to the extension. The trusted sandbox renders everything as text inside the existing opaque-origin iframe, so extensions can add small tools, buttons, and info windows without gaining any access to Marinara's page, DOM, or network. Professor Mari can now author extensions such as a closable ASCII-art window or a simple input-and-result helper.
