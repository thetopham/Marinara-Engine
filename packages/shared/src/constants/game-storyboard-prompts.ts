...
export const GAME_STORYBOARD_PROMPT_DIRECTOR_TEMPLATE = [
  ...
  "Create exactly ${keyframeCount} shots when the narration contains enough distinct visual beats. For a shorter turn, return fewer shots rather than duplicating moments, padding the plan, or inventing events.",
  "When keyframeCount is greater than 1, divide the source into distinct chronological shot ranges and return one independent keyframe object for each range. Every keyframe must have its own imagePrompt and videoPrompt; never collapse the whole turn into one keyframe or reuse one illustration for several ranges.",
  "Storyboard segmentation example: for six chronological source sections and keyframeCount 3, keyframe 1 may cover sections 0-1, keyframe 2 sections 2-3, and keyframe 3 sections 4-5. Move the boundaries to the narration's actual visual beats and do not copy these example indices when the source differs.",
  ...
  "Treat the supplied canonical appearance context as authoritative. Do not redesign a character, invent an alternate outfit, or add extra anatomy, tails, weapons, equipment, or decorative duplicates.",
  "Put any unresolved contradiction between the narration and canonical visual context in warnings. Otherwise return an empty warnings array; never silently invent a compromise design.",
  "Write narrationBeat as a short human-readable summary of the selected visual beat.",
  "Write imagePrompt as time T=0: the exact first frame immediately before or as that keyframe's action begins. Each imagePrompt is rendered as its own storyboard illustration. Describe visible subjects, starting pose and expression, composition, camera position, environment, lighting, palette, textures, and important props, but do not include later consequences or animation instructions.",
  ...
  "For clips with several connected actions, divide videoPrompt into 2-4 compact inline time ranges while preserving one continuous spatially clear shot. Prefer one strong causal action chain over several unrelated actions or complex physics.",
  'Timed-segment example for an illustrative six-second shot: "0-2s: with a locked medium-wide camera, the subject reaches toward the control. 2-4s: their hand closes around it and the mechanism responds with a pulse of light. 4-6s: the light settles as the subject holds the completed pose." Scale the boundaries to ${durationSeconds} seconds, replace every example event with source-supported action, and keep an explicitly locked camera when no movement is needed.',
