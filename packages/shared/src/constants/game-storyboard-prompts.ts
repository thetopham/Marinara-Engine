...
export const GAME_STORYBOARD_PROMPT_DIRECTOR_JSON_SHAPE_LINE =
  '{ "title": string, "summary": string, "referenceSheetPrompt": string, "warnings": string[], "keyframes": [ { "title": string, "sectionStartIndex": number, "sectionEndIndex": number, "anchorQuote": string, "anchorKind": "narration" | "dialogue" | "readable" | "system", "narrationBeat": string, "imagePrompt": string, "videoPrompt": string, "characters": string[], "continuityNotes": string, "cameraMotion": string, "transitionHint": string, "durationSeconds": number, "aspectRatio": "16:9" | "9:16" } ] }';
...
export const GAME_STORYBOARD_PROMPT_DIRECTOR_TEMPLATE = [
  ...
  "Write referenceSheetPrompt as one standalone 16:9 production reference-sheet prompt for the important visible subjects, signature equipment, props, creature anatomy, and scale relationships used by these shots. Use a clean editorial grid on a plain neutral background with separated full-body, front/side/back turnaround, expression, material/detail, prop, palette, and scale panels only where the source supports them.",
  "Ground the sheet in the supplied canonical appearance context. Keep every subject anatomically consistent across panels, show exactly one canonical design per subject, and do not invent alternate outfits, extra limbs, extra tails, extra weapons, story events, scenery, or decorative duplicates. Use only short functional labels when needed and no paragraphs of rendered text.",
