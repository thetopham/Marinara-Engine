...
export const GAME_STORYBOARD_COMIC_ANIMATION_PROMPT_TEMPLATE = [
  ...
  "Budget roughly one major panel for every 3 seconds of clip time: use 1-2 panels for 1-5 seconds, 2 panels for 6-7 seconds, 2-3 panels for 8-10 seconds, and 3-4 panels for 11-15 seconds. Never exceed 4 panels, and use fewer whenever the source has fewer distinct beats.",
  "Use only the GM narration as the story source. Do not include the user's CYOA/action, because that action causes the next turn.",
  "Use the supplied turn_sections indices to anchor every keyframe to the story text. Prefer contiguous section ranges that cover the whole turn in order.",
  "For each keyframe, set sectionStartIndex and sectionEndIndex to the first and last covered section indices. Set anchorQuote to a short exact phrase from those sections, and anchorKind to the dominant section kind.",
  ...
  "Use expressive speech bubbles, captions, and SFX lettering only when supported by the source. Keep text short and exact; never paraphrase dialogue into a different meaning or invent words for silent characters.",
  "Write narrationBeat as a compact animation plan that uses the comic page as an ordered temporal reference. Allocate the full ${durationSeconds} seconds with natural-language time ranges, identify the primary subject motion, one simple camera move or panel transition at a time, subtle secondary environmental motion, and the final hold. Do not ask the video model to animate every panel at once.",
